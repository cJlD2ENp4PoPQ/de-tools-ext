...
5. **Script load order in `manifest.json` is significant.** Utilities must appear before the page scripts that depend on them. When adding a new page script, append it at the end of the `js` array. When adding a new utility, insert it before all page scripts.
...
9. **When adding new storage keys**, also update `STORAGE_KEYS` in `options/settings.js` and add the corresponding `.category-card` block to `options/settings.html`.
...
js/utils/     — shared utility modules (Storage, fields, Tables, frame, Time, ContextMenu)
